...
function truffleConsole() {
    ...
    const addresses = require("witnet-solidity-bridge/migrations/witnet.addresses")[chain[0]][chain[1]]
...
function traceWitnetAddresses(addresses, selection) {
    let found = 0
    for (const key in addresses) {
        if (typeof addresses[key] === "object") {
            found += traceWitnetAddresses(addresses[key], selection)
        } else {
            if (
                (selection.length === 0
                    && key !== "Create2Factory"
                ...
                found ++
                const tab = key.length >= 28 ? "" : key.length >= 20 ? "\t" : "\t\t";
                console.info("  ", key, tab, "=>", addresses[key])
            }
        }
    }
    return found
